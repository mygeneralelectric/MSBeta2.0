                            ...
                            <c:if test="${not empty users}">
                                <c:forEach items="${users}" var="user">
                                    <tr>
                                        ...
                                        <sec:authorize access="hasRole('ADMIN')">
                                            <td>
                                                <a href="<c:url value='/edit-user-${user.ssoId}' />" class="waves-effect waves-light btn">修改</a>
                                            </td>
                                            <td>
                                                <a href="#${user.ssoId}" class="waves-effect waves-light btn modal-trigger">删除</a>
                                                    <%--<a href="<c:url value='/delete-user-${user.ssoId}' />" class="waves-effect waves-light btn modal-trigger">删除</a>--%>

                                                <div id="${user.ssoId}" class="modal">
                                                    <div class="modal-content">
                                                        <h4>确认删除${user.ssoId}？</h4>
                                                        <p>一旦删除，无法撤销！确定想要删除？</p>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <a href="#" class=" modal-action modal-close waves-effect waves-green btn-flat">取消</a>
                                                        <a href="<c:url value='/delete-user-${user.ssoId}' />" class=" modal-action modal-close waves-effect waves-green btn">确认</a>
                                                    </div>
                                                </div>
                                            </td>
                                        </sec:authorize>
